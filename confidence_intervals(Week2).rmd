...
```{r set-seed}
set.seed(200796)                 # make sure to change the seed
```
...
samp %>% summarise(x_bar=mean(area),s=sd(area),med_samp = median(area),min_samp = min(area), max_samp = max(area),iqr_samp=IQR(area),q1_samp=quantile(area,0.25),q2_samp=quantile(area,0.75))
```
...
# type your code for the Exercise here, and Knit
 samp %>% summarize(lower=mean(area)-z_star_95*(sd(area)/sqrt(n)),upper = mean(area) + z_star_95 * (sd(area) / sqrt(n)))
```
