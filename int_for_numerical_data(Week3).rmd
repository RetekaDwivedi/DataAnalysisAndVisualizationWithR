...
ans. Both distributions are extremely right skewed
```{r habit-weight-box}
...
ans. $H_0: \mu_{smoking} = \mu_{non-smoking}$; $H_A: \mu_{smoking} \ne \mu_{non-smoking}$ 
...
ans. We are 95% confident that babies born to nonsmoker mothers are on average 0.05 to 0.58 pounds heavier at birth than babies born to smoker mothers
```{r habit-weight-ci}
...
```{r pick-your-own}
# type your code for the Exercise here, and Knit
nference(y = visits, x = premie, data = nc, statistic = "mean", type = "ht", null = 0, alternative = "greater", method = "theoretical")
```
